...
shortVowelsDoNotMatter = True
# change to FASLE if words vocalized differently in DIPL and EDIT should
# go into the ctitical apparatus
	...
	with open(pathToFile, "r", encoding="utf8") as f1:
		...
		for t in text[0:]:
			...
			if t.startswith(lineStarter):
				...
				if len(t) == 2:
					...
					if "«" in dipl:
						...
						lineCounterA = "%da" % lineCounter
					else:
						lineCounter += 1
						lineCounterA = "%d" % lineCounter
					newLine = ["<span class='line'>|%s|</span> " % lineCounterA]
			...
			# process 
				...
				insertID   = t[3:].split("::")[0].strip()
				insertBody = t[3:].split("::")[1].strip()
				footnotesData[insertID] = "<i>insert</i>: " + insertBody
		...
		for i in re.finditer(r"\b([^\s]+\[\[[^\]]+\]\])", textFinal):
			...
			if shortVowelsDoNotMatter == True:
				matches = matchV.replace("]]", "").split("[[")
				#input(matches)
				if deNoise(matches[0]) == deNoise(matches[1]):
					print("the same...")
					textFinal = textFinal.replace(matchV, matches[0])
				else:
		...
		# correct between-the-line insertions
		textFinal = textFinal.replace("«", "<span class='betweenTheLines'>|")
		textFinal = textFinal.replace("»", " |</span>")
...
converter("/Users/romanov/_OpenITI/mARkdownMSS/1280CumarIbnSayyid_2/1280CumarIbnSayyid.SuratNasr.UNCVCPBM837KD8-ara1.mARkdownMSS")
