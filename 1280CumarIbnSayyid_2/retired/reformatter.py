	...
	with open(pathToFile, "r", encoding="utf8") as f1:
		...
		for t in text[0:]:
			...
			# process 
				...
				insertnotesData[insertID] = "<i>insert</i>: " + insertBody
		...
		variants = []
		inCounter = 0
		for k,v in insertnotesData.items():
			#refMap = textFinal.count(k)
			refMap = len(re.findall(r"\b%s\b" % k, textFinal))
			#print(refMap)
			...
			if refMap == 1:
				...
				counterTemp = 0
				print(k)
				m1 = "<span class='comment'>^^%d^^</span>" % inCounter
				textFinal = re.sub(r"\b%s\b" % (k), r"%s" % (m1), textFinal)

				#footNote = m1 + " ... " + m2 + " : " + v + ".<br>"
				insertNote = m1 + " : " + v + "; <br>"
				insertnotesFinal.append(insertNote)
		...
		inserts   = "<h3>Insertions</h3>\n\n" + "<p class='comments'>\n\n%s\n\n</p>" % "\n".join(insertnotesFinal)
		comments  = "<h3>Comments</h3>\n\n" + "<p class='comments'>\n\n%s\n\n</p>" % "\n".join(footnotesFinal)
		variants  = "<h3>Variants</h3>\n\n" + "<p class='variants'>\n\n%s\n\n</p>" % "\n".join(variants)
		metadata  = "<h3>Metadata</h3>\n\n" + "<p class='metadata'>\n\n%s\n\n</p>" % metadata

		textFinal = textFinal + variants + inserts + comments + metadata
